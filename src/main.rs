...
fn compare_print_hypothesis<PSA: ParticleSource, PSB: ParticleSource>(mut particle_source_a: PSA, mut particle_source_b: PSB, repetitions: u32) {

    let error = 0.8;
    let mut c_updown_single = QCircuitFactory::series(1, FilterType::UpDown);
    c_updown_single.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);

    let mut c_leftright_single = QCircuitFactory::series(1, FilterType::LeftRight);
    c_leftright_single.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);

    let mut c_updown_series = QCircuitFactory::series(3, FilterType::UpDown);
    c_updown_series.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);

    let mut c_leftright_series = QCircuitFactory::series(3, FilterType::LeftRight);
    c_leftright_series.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);

    let mut c_tree2 = QCircuitFactory::tree(2, FilterType::UpDown);
    c_tree2.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);

    let mut c_tree3 = QCircuitFactory::tree(3, FilterType::LeftRight);
    c_tree3.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);

    let mut c_tree4 = QCircuitFactory::tree(4, FilterType::UpDown);
    c_tree4.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);

    let mut c_tree5 = QCircuitFactory::tree(5, FilterType::UpDown);
    c_tree5.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);

    let mut c_tree6 = QCircuitFactory::tree(6, FilterType::UpDown);
    c_tree6.assert_compare(&mut particle_source_a, &mut particle_source_b, repetitions, error);
}
